	...
	public static void main(String[] args) throws Exception {
		...
		while(tc-->0){
			...
			for(int i = 0 ; i<n*2 ; i +=2){
				...
				if(x<x1){
				a[i] =new Pair(x, mag) ;
				a[i+1] =new Pair(x1, 1/mag) ; 
				}else{
					a[i+1] =new Pair(x1, mag) ;
					a[i] =new Pair(x, 1/mag) ;
				}
			}
			Arrays.sort(a);
			pw.printf("%d\n-inf %.3f 1.000\n" , n*2+1, a[0].first);
			...
			for(int i = 0 ; i< (n*2)-1 ; i ++){
				double  curr = a[i].first ; 
				double mag = a[i].second ; 
				double tmp = ans ; 
				ans *=mag ; 
				if(ans!=tmp){
					counter++ ;
				pw.printf("%.3f %.3f %.3f\n", a[i].first  , a[i+1].first , ans);				
				}
			}
			if(tc!=0)
			pw.printf("%.3f +inf 1.000\n\n" , a[n*2-1].first);
			else pw.printf("%.3f +inf 1.000\n" , a[n*2-1].first);
