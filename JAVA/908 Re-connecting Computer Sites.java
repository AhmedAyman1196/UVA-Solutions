...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		while(true){
			// input
			long initialCost = 0  ;
			for(int i = 0 ; i < n-1 ; i ++){ // previously chosen
			...
			int k = sc.nextInt() ;
			for(int i = 0 ; i < k ; i ++){
				int from = sc.nextInt()-1  , to = sc.nextInt()-1 , weight = sc.nextInt();
			...
			UnionFind uf = new UnionFind(n+10000) ;
			long finalCost = 0 ;
			for(Edge e : edgeList){
			...
			pw.println(initialCost);
			pw.println(finalCost);
			if(!sc.ready())break ;
			n = sc.nextInt();
			pw.println();
		}
